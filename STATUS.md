...
- `prompts.ts`, `levelFromSketch.ts`: one structured-output call (photo + system prompt, schema from `z.toJSONSchema(LevelSchema)`), then sanitize -> `validateLevel`. If unreachable it sends the photo, the level JSON and the validator report back for minimal edits (max 2 rounds). Level generation has **no time limit**: no per-call timeout, no scan budget, and repairs are never skipped for time (the `GEMINI_LEVEL_*_MS` env vars are gone). They were 6s / 4s / 8s, then 120s / 120s / 300s, and a timeout only ever produced a fallback level. `generate()` takes an optional `timeoutMs`; only the roast still passes one. A Cloudflare quick tunnel still cuts any request at ~100s, and the client (`fetch`) has no timeout of its own to remove. Stage timings are logged.
